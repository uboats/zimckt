...
global S_type DC_ TRAN_ AC_ DEC_ LIN_ DC_SWEEP
...
for m=1:size(plotnv,1)
    nv = combp(num2str(nodes(plotnv(m))),nv,m);%[nv;num2str(nodes(plotnv(m)))];%
end
bi=[];
for m=1:size(plotbi,1)
    bi = combp(num2str(nodes(plotbi(m))),bi,m);%[bi;num2str(names(plotbi(m,6),:))];
end
